...
{
	$(document).mousewheel(wheelVolControl);
}
...
function radioPlay(channel)
{
	...
	if (playerReady) {
		...
		$('#player-back').addClass('rotate-back');
...
function radioStop()
{
	if (playerReady && radioPlayer)
	{
		...
		$('#player-back').removeClass('rotate-back');
		setVal('player_on', 0);
	}
}
...
function requestTrackInfo()
{
    ...
    $.ajax(
    {
        url: 'https://bits.waveradio.org/api/track/nyan',
        dataType: 'json',
        crossDomain: true
    }).done(
        function (data)
